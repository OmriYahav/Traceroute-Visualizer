...
def get_location_data(ip_address):
    ...
    try:
        ...
        logger.error(
            f"HTTP error occurred while getting location data for {ip_address}: {e}"
        )
        return None
    except requests.exceptions.RequestException as e:  # handle other request errors
        logger.error(
            f"Request error occurred while getting location data for {ip_address}: {e}"
        )
        return None
    ...
    try:
        ...
        return (
            ...
            latitude,
            longitude,
        )  # return the address, latitude, and longitude as a tuple
    except (
        KeyError,
        TypeError,
    ) as e:  # handle errors in parsing the JSON or reverse geocoding
        logger.error(
            f"Error occurred while parsing location data for {ip_address}: {e}"
        )
        return None
...
def traceroute_route():
    dest_addr = request.form[
        "destination_address"
    ]  # get the destination address from the form data
    start_time = time_ns()  # get the current time in nanoseconds
    logger.info(
        f"Starting traceroute to {dest_addr} at {start_time // (10 ** 9)} seconds"
    )  # log the start of the traceroute
    ans, unans = traceroute(
        dest_addr
    )  # perform the traceroute and get the answered and unanswered packets
    ...
    color_index = (
        0  # initializes a color index to keep track of which color to use next
    )
    map_location = folium.Map(
        location=[0, 0], zoom_start=2
    )  # creates a new map with a starting location of [0,0] and zoom level of 2

    for i, hop in enumerate(
        ans
    ):  # iterates through each answered packet in the traceroute
        ...
        if ip_address not in unique_hops:  # checks if the IP address is unique
            ...
            location_data = get_location_data(
                ip_address
            )  # retrieves the latitude and longitude of the current hop from an external API
            if location_data:  # checks if the location data was successfully retrieved
                address, latitude, longitude = (
                    location_data  # unpacks the location data into variables
                )
                logger.info(
                    f"Hop {i + 1}: {ip_address}, {address}, Latitude: {latitude}, Longitude: {longitude}"
                )  # logs the location data for the current hop
                coordinates.append(
                    [latitude, longitude]
                )  # adds the coordinates to the list of coordinates
                folium.Marker(
                    location=[
                        latitude,
                        longitude,
                    ],  # adds a marker to the map for the current hop
                    ...
                    icon=folium.Icon(
                        color="green", icon_color="white", icon="ok-circle"
                    ),
                ).add_to(
                    map_location
                )  # sets the marker's color and icon
                if (
                    i < len(ans) - 1
                ):  # checks if this is not the last hop in the traceroute
                    next_hop = ans[i + 1][
                        1
                    ].src  # gets the source IP address of the next hop
                    next_hop_location = get_location_data(
                        next_hop
                    )  # retrieve location data once
                    if next_hop_location:
                        ...
                        logger.warning(
                            f"Could not retrieve location data for next hop {next_hop}"
                        )
                # If location data is not available, log it and continue to the next hop

            else:
                logger.info(
                    f"Hop {i + 1}: {ip_address} - could not retrieve location data"
                )
